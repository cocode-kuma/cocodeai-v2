import { openworkCloudMcpInlineReconnectSchema } from "@cocodeai/types/den/mcp-connection-action"
...
export function reconnectActionFromChatToolResult(
  ...
  // Tool output is otherwise untrusted. Only the two canonical CocodeAI Cloud
...
export function attributeChatToolError(errorText: string): ToolErrorAttribution | null {
  ...
  if (
    errorText.includes("CocodeAI stopped waiting after")
    ...
    return confirmed(
      "CocodeAI timeout",
      "CocodeAI created this deadline. The external operation may still have completed, so verify its state before retrying.",
    )
  }
  ...
  if (
    ...
    return confirmed("Blocked by CocodeAI", "CocodeAI blocked the request before it was sent.")
  }
