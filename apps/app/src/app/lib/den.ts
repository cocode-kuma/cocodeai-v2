...
export const DEFAULT_DEN_AUTH_NAME = "CocodeAI User";
...
export const HOSTED_DEFAULT_DEN_BASE_URL = "https://app.cocodeai.com";
...
/**
 * True when the effective Den control plane is not the hosted CocodeAI Cloud
 * (app.cocodeai.com). Self-hosted deployments point the app at their own
 * control plane via VITE_DEN_BASE_URL or the desktop bootstrap config, so
 * hosted-only surfaces (e.g. CocodeAI Models upsells) should stay hidden.
 */
 ...
 * web-app origin (e.g. `https://app.cocodeai.com/mcp`). Nothing serves
 ...
 * (`https://app.cocodeai.com/mcp`) where nothing serves MCP — heal
...
export function buildDenAuthUrl(baseUrl: string, mode: "sign-in" | "sign-up"): string {
  ...
  if (isDesktopDeployment()) {
    ...
    target.searchParams.set("desktopScheme", "cocodeai");
  }
