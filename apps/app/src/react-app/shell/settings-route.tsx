...
import { AboutView } from "@/react-app/domains/settings/pages/about-view";
...
function isCocodeAICloudProvider(provider: {
...
export function parseSettingsPath(pathname: string): {
  ...
  switch (head) {
    ...
    case "memory":
      // CocodeAI: MCP/Cloud 页面已隐藏，重定向到 general
      return { tab: "general", redirectPath: "general" };
    case "den":
    case "cloud-workers":
      return { tab: "general", redirectPath: "general" };
...
function SettingsRouteContent(props: SettingsSurfaceProps = {}) {
  ...
  const hasCocodeAICloudProvider = useMemo(
    () =>
      providerAuthSnapshot.cloudOrgProviders.some(isCocodeAICloudProvider) ||
      Object.values(providerAuthSnapshot.importedCloudProviders ?? {}).some(isCocodeAICloudProvider),
  ...
  const openWorkModelsConnected =
    (cloudSession.isSignedIn && hasCocodeAICloudProvider) ||
  ...
  const installOpenAiImageExtension = useCallback(async (apiKey: string) => {
    ...
    if (!openworkClient) {
      setImageExtensionError("CocodeAI server is not connected.");
      return;
    }
    if (!resolvedApiKey) {
    ...
    try {
      ...
      setImageExtensionStatus("Saved OPENAI_API_KEY. Agents can use CocodeAI extension actions for image generation.");
    } catch (error) {
  ...
  const generateOpenAiTestImage = useCallback(async (input: { apiKey: string; prompt: string }) => {
    ...
    if (!client || !workspaceId) {
      setImageGenerationError("CocodeAI server is not connected for this workspace.");
      return;
    }
    if (!apiKey) {
  ...
  const testVoiceSession = useCallback(async () => {
    if (!openworkClient) {
      setVoiceError("CocodeAI server is not connected.");
      return;
    }
    setVoiceBusy(true);
    setVoiceStatus(null);
    ...
    try {
      ...
      setVoiceStatus(`Realtime ready with ${session.model} (${session.tools.length} CocodeAI tools).`);
    } catch (error) {
      setVoiceError(describeRouteError(error));
    } finally {
      setVoiceBusy(false);
  ...
  const installLocalProvider = useCallback(async (input: LocalProviderInstallInput) => {
    ...
    if (!client || !workspaceId) {
      setLocalProviderError("CocodeAI server is not connected for this workspace.");
      return;
    }
    if (!modelId) {
  ...
  const settingsView = (() => {
    switch (route.tab) {
      ...
      case "memory":
      case "cloud-providers":
        // CocodeAI: Cloud/Marketplace/Memory 页面已隐藏
        if (!props.embedded) {
