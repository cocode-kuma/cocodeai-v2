    ...
    void (async () => {
      try {
        ...
        const startServerWithoutDesktopWorkspace = async () => {
          setPhase("starting-engine", "Starting CocodeAI server");
          ...
          if (!isOpenworkServerInfoLike(serverInfo) || !isOpenworkServerReady(serverInfo)) {
            setError("CocodeAI server did not finish starting. Please restart CocodeAI.");
            return;
          }
        ...
        if (isElectronRuntime()) {
          ...
          if (boot.ok === false) {
            setError(boot.error || "Failed to start CocodeAI runtime");
            return;
          }
