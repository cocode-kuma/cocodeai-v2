...
const kindDesc: Record<ExtensionKind, string> = {
  ...
  plugin: "Extends CocodeAI with additional capabilities managed by your organization.",
  skill: "A reusable workflow that your agent can execute on demand.",
  "ui-control": "Lets another MCP client inspect and drive this CocodeAI desktop UI through a local stdio wrapper.",
  extension: "A CocodeAI extension that adds tools, providers, or integrations to your workspace.",
};
                ...
                <CardContent>
                  <div className="space-y-3 text-sm">
                    ...
                    {contributionLabels.length > 0 ? (
                      <div>
                        <div className="mb-1 text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">贡献</div>
                        <div className="flex flex-wrap gap-1.5">
                ...
                <div className="space-y-2">
                  ...
                  {preview ? (
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium text-blue-11">预览</span>
                    </div>
                  ) : null}
...
function UiControlConnectionDetails(props: UiControlConnectionDetailsProps) {
  ...
  return (
    <div className="space-y-4">
      <Card variant="outline" size="sm">
        <CardHeader>
          <CardTitle>如何连接其他客户端</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col gap-2 text-sm leading-relaxed text-muted-foreground">
            <div>CocodeAI 桌面端会自动启动一个私有的本地桥接服务。</div>
            <div>你的 MCP 客户端通过 stdio 启动 <span className="font-mono text-card-foreground">openwork-ui-mcp</span>；包装器会自动发现桥接并将 UI 工具代理到其上。</div>
            <div>请不要直接将客户端指向随机的 localhost 桥接 URL。</div>
          </div>
        </CardContent>
      </Card>
