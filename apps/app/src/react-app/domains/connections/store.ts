...
export function createConnectionsStore(options: {
  ...
  const listMcpFromOpenworkServer = async (projectDir: string) => {
    ...
    if (hasOpenworkTarget && !canTryOpenworkServer) {
      throw new Error("CocodeAI server cannot read MCP config for this workspace.");
    }
    ...
    try {
      ...
      if (!fallbackOnError) {
        throw error instanceof Error
          ? error
          : new Error("Computer Use helper app is unavailable. Restart CocodeAI or reinstall the app.");
      }
  ...
  async function refreshMcpServers() {
    ...
    if (isRemoteWorkspace) {
      mutateState((current) => ({
        ...current,
        mcpStatus: "CocodeAI server unavailable. MCP config is read-only.",
        mcpServers: [],
        mcpStatuses: {},
      }));
      return;
  ...
  async function connectMcp(entry: McpDirectoryInfo): Promise<boolean> {
    ...
    if (isRemoteWorkspace && !canUseOpenworkServer) {
      setStateField("mcpStatus", "CocodeAI server unavailable. MCP config is read-only.");
    ...
    if (hasOpenworkTarget && !canUseOpenworkServer) {
      setStateField("mcpStatus", "CocodeAI server MCP config is read-only.");
      finishPerf(options.developerMode(), "mcp.connect", "blocked", startedAt, {
    ...
    try {
      ...
      if (entry.serverName === CLOUD_MCP_SERVER_NAME) {
        if (!canUseOpenworkServer || !openworkClient || !openworkWorkspaceId) {
          throw new Error("CocodeAI server is required to repair agent access to connected services.");
        }
        const context = await resolveCloudMcpOperationContext(entry.url);
        if (!context) {
          throw new Error("Sign in to CocodeAI Cloud and choose an organization first.");
        }
      ...
      if (entryType === "remote") {
        if (!resolvedUrl) {
          throw new Error("Missing MCP URL. Is the CocodeAI desktop app running?");
        }
  ...
  async function logoutMcpAuth(name: string) {
    ...
    if (isRemoteWorkspace && !canUseOpenworkServer) {
      setStateField("mcpStatus", "CocodeAI server unavailable. MCP auth is read-only.");
      return;
    }

    if (hasOpenworkTarget && !canUseOpenworkServer) {
      setStateField("mcpStatus", "CocodeAI server MCP auth is read-only.");
      return;
    }
