...
export async function waitForFreshMcpAuthorization(input: {
  ...
  while (now() - startedAt < timeoutMs) {
    if (!input.isScopeCurrent()) {
      throw new Error("The active CocodeAI Cloud account changed while reconnecting. Try again in this workspace.")
    }
    try {
      ...
      if (error instanceof Error && error.message.startsWith("The active CocodeAI Cloud account changed")) throw error
