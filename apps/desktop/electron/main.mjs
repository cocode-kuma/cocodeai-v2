...
const NATIVE_DEEP_LINK_EVENT = "cocodeai:deep-link-native";
const TAURI_APP_IDENTIFIER = "com.cocodeai.cocodeai";
const DEV_APP_IDENTIFIER = "com.cocodeai.cocodeai.dev";
const DESKTOP_PROTOCOL_SCHEME = "cocodeai";
const isDevMode = process.env.COCODEAI_DEV_MODE === "1";
const APP_NAME =
  process.env.COCODEAI_ELECTRON_APP_NAME?.trim() ||
  (isDevMode ? "CocodeAI - Dev" : "CocodeAI");
let currentDisplayAppName = APP_NAME;
const APP_IDENTIFIER =
  process.env.COCODEAI_ELECTRON_APP_IDENTIFIER?.trim() ||
  (isDevMode ? DEV_APP_IDENTIFIER : TAURI_APP_IDENTIFIER);
if (process.env.COCODEAI_ELECTRON_USE_MOCK_KEYCHAIN === "1") {
...
//
// Override via COCODEAI_ELECTRON_USERDATA so dogfooders can isolate their
// Electron install from the real Tauri app.
app.setName(APP_NAME);
app.setAppUserModelId(APP_IDENTIFIER);
if (app.isPackaged && process.env.COCODEAI_ELECTRON_DISABLE_PROTOCOL_REGISTRATION !== "1") {
  app.setAsDefaultProtocolClient(DESKTOP_PROTOCOL_SCHEME);
}
const userDataOverride = process.env.COCODEAI_ELECTRON_USERDATA?.trim();
if (userDataOverride) {
...
async function resolveArchitectureInfo() {
  ...
  const assetName = `cocodeai-${platformDownloadSlug()}-${downloadAssetArch(targetArch)}-${version}.${downloadAssetExtension()}`;
...
function defaultAppWindowsIconPath() {
  return path.join(app.getPath("userData"), "cocodeai-stock.ico");
}
...
/**
 * @param {unknown} input
 * @returns {import("@cocodeai/types/desktop-ipc").DesktopNotificationResult}
 */
...
// drive the built-in browser panel.  Use COCODEAI_ELECTRON_REMOTE_DEBUG_PORT to
...
const explicitCdpPort = Number.parseInt(
  process.env.COCODEAI_ELECTRON_REMOTE_DEBUG_PORT?.trim() ?? "",
  10,
);
...
// agent instructions via ensureCocodeaiAgent → resolveAgentTemplate.
process.env.COCODEAI_ELECTRON_REMOTE_DEBUG_PORT = String(remoteDebugPort);
...
const IDLE_COCODEAI_SERVER_INFO = Object.freeze({
  running: false,
...
/**
 * @param {string} rawUrl
 * @returns {import("@cocodeai/types/connect-link").ConnectLinkVerifyResult}
 */
...
function forwardedDeepLinks(argv) {
  return argv
    ...
    .filter(
      (entry) =>
        entry.startsWith("cocodeai://") ||
        entry.startsWith("cocodeai-dev://") ||
...
<html>
  ...
  <body>
    <main>
      ...
      <div class="title">Stopping CocodeAI services</div>
...
function assertCocodeaiServerReady(info) {
  if (!info?.running) {
    throw new Error("CocodeAI server did not stay running after startup.");
  }
  if (!info.baseUrl) {
    throw new Error("CocodeAI server did not report a base URL after startup.");
  }
  if (!info.ownerToken && !info.clientToken) {
    throw new Error("CocodeAI server did not report an access token after startup.");
  }
  return info;
}
...
// extra, or renamed command fails `pnpm --filter @cocodeai/desktop
// typecheck:electron`.
/** @type {import("@cocodeai/types/desktop-ipc").DesktopCommandHandlers<import("electron").IpcMainInvokeEvent>} */
const desktopCommandHandlers = {
  ...
  "appBuildInfo": async (event, ...args) => {
      return {
        ...
        cocodeaiDevMode: process.env.COCODEAI_DEV_MODE === "1",
      };
  },
