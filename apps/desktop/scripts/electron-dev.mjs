...
const explicitStartUrl = process.env.COCODEAI_ELECTRON_START_URL?.trim() || "";
...
console.log("[electron-dev] Building cocodeai-server (tsc)...");
runSync(pnpmCmd, ["--filter", "cocodeai-server", "build"], { cwd: repoRoot });
...
if (cdpPort) {
  console.log(`[cocodeai] Electron CDP exposed at http://127.0.0.1:${cdpPort}`);
}
