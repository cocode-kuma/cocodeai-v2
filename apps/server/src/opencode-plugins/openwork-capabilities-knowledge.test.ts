...
import { CocodeAICapabilitiesKnowledge } from "./openwork-capabilities-knowledge.js";

describe("OpenWork capabilities knowledge plugin", () => {
  test("injects current OpenWork Connect guidance", async () => {
    const plugin = await CocodeAICapabilitiesKnowledge();
    const output = { system: [] };
  ...
  test("retrieves Slack connection guidance from bundled docs", async () => {
    ...
    const plugin = await CocodeAICapabilitiesKnowledge();
    const search = await plugin.tool.cocodeai_docs_search.execute({ query: "how can i connect slack", limit: 3 });
    ...
    const read = await plugin.tool.cocodeai_docs_read.execute({
  ...
  test("retrieves the Connect-first member flow from bundled docs", async () => {
    ...
    const plugin = await CocodeAICapabilitiesKnowledge();
    const search = await plugin.tool.cocodeai_docs_search.execute({ query: "connect gmail calendar slack", limit: 3 });
