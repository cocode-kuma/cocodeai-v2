...
export async function listSkills(workspaceRoot: string, includeGlobal: boolean): Promise<SkillItem[]> {
  ...
  for (const root of roots) {
    ...
    const cocodeDir = join(root, ".cocode", "skills");
    items.push(...(await listSkillsInDir(opencodeDir, "project")));
    items.push(...(await listSkillsInDir(cocodeDir, "project")));
  }

  if (includeGlobal) {
    const globalOpenWork = join(homedir(), ".config", "opencode", "skills");
    const globalCocode = join(homedir(), ".cocode", "skills");
    ...
    items.push(...(await listSkillsInDir(globalCocode, "global")));
