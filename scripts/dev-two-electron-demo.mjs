...
export function resolveDemoRoot(env = process.env) {
  return env.COCODEAI_ELECTRON_DEMO_ROOT?.trim() || path.join(os.tmpdir(), "cocodeai-two-electron-demo");
}

const demoRoot = resolveDemoRoot();
const appProfiles = {
  admin: {
    appIdentifier: "com.cocodeai.demo.admin",
    appName: "CocodeAI Demo A",
  ...
  consumer: {
    appIdentifier: "com.cocodeai.demo.consumer",
    appName: "CocodeAI Demo B",
...
function profilePaths(runRoot, profile) {
  ...
  return {
    ...
    dataDir: path.join(root, "cocodeai-data"),
    dataHome: path.join(root, "xdg-data"),
    envStorePath: path.join(root, "cocodeai-env.json"),
...
async function main() {
  ...
  if (built && !existsSync(path.join(repoRoot, "apps", "app", "dist", "index.html"))) {
    throw new Error("The desktop renderer is not built. Run pnpm --filter @cocodeai/desktop build:electron first.");
  }
