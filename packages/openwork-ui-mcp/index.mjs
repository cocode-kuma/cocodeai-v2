 ...
 * cocodeai-ui-mcp
 *
 * MCP server that exposes CocodeAI's UI control surface as MCP tools.
 * Speaks MCP stdio and proxies to the CocodeAI desktop bridge HTTP API.
 *
 * Requires CocodeAI desktop running with the local UI control bridge active.
 *
 * Usage:
 *   npx cocodeai-ui-mcp
 *
...
const DISCOVERY_FILE = "cocodeai-ui-control.json";
...
function discoveryPaths() {
  return [
    ...
    join(userAppDataDir(), "com.cocodeai.cocodeai", DISCOVERY_FILE),
    join(userAppDataDir(), "com.cocodeai.cocodeai.dev", DISCOVERY_FILE),
  ].filter(Boolean);
}
...
async function bridgeRequest(path, options = {}) {
  ...
  if (!bridge) {
    return {
      ok: false,
      error: "CocodeAI is not running. Launch the CocodeAI desktop app first.",
      hint: "The MCP server connects to a running CocodeAI instance via its local bridge.",
    };
  }
...
server.tool(
  "ui_snapshot",
  "Get a snapshot of the current CocodeAI UI state: active route, narration, visible actions, and status. Use this before taking action to understand what the user sees.",
  {},
  async () => {
    ...
    return { content: [{ type: "text", text: lines.join("\n") || "CocodeAI is reachable, but it did not return visible UI state." }] };
  }
);
...
server.tool(
  "ui_list_actions",
  "List all UI control actions currently available in CocodeAI: session navigation, composer control, transcript access, and more. Each action has an id you can pass to ui_execute_action.",
  {},
  async () => {
    ...
    if (!Array.isArray(result.actions) || result.actions.length === 0) {
      return { content: [{ type: "text", text: "No actions available. Is CocodeAI on the main screen?" }] };
    }
...
server.tool(
  "ui_execute_action",
  "Execute an CocodeAI UI action by its id. Use ui_list_actions first to see available actions and their required arguments.",
  {
...
server.tool(
  "ui_status",
  "Check if CocodeAI is running and the bridge is reachable. Returns connection status and app info.",
  {},
  async () => {
    const bridge = await discoverBridge();
    if (!bridge) {
      return { content: [{ type: "text", text: "CocodeAI is not running.\nLaunch the CocodeAI desktop app to enable UI control." }], isError: true };
    }
    try {
      const response = await fetch(`${bridge.baseUrl}/health`, { signal: AbortSignal.timeout(3000) });
      const data = await response.json();
      return { content: [{ type: "text", text: `Connected to ${data.app || "CocodeAI"}\nBridge: ${bridge.baseUrl}\nVersion: ${data.version ?? "?"}` }] };
    } catch (error) {
      clearBridgeCache();
      return { content: [{ type: "text", text: `Bridge file found but not reachable: ${error.message}\nCocodeAI may have quit. Relaunch it.` }], isError: true };
    }
  }
);
