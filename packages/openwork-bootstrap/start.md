...
You are an agent helping a user install and set up CocodeAI.

Goal: install CocodeAI, create a provisional workspace without requiring email
...
offer to connect the CocodeAI MCP to the agent the user is already using.

> The bootstrap command is `cocodeai-bootstrap` (NOT `cocodeai`). The bare
> `cocodeai` command belongs to the separate `cocodeai-orchestrator` package and
> may already be on the user's PATH - do not use it for these steps.

## 1. Install CLI

Install the `cocodeai-bootstrap` CLI into a user-writable bin dir.

If you already have this repository locally:

```bash
node packages/cocodeai-bootstrap/bin/cocodeai.mjs install \
  --install-dir "$HOME/.cocodeai/bootstrap" \
...
`cocodeai-bootstrap` CLI (a single dependency-free Node file) and installs it
into `$HOME/.local/bin` - no npm or npx required.

```bash
curl -fsSLo /tmp/cocodeai-install.sh https://cocodeailabs.com/install.sh
less /tmp/cocodeai-install.sh
sh /tmp/cocodeai-install.sh
```

Verify:

```bash
cocodeai-bootstrap doctor --json
```
...
```bash
cocodeai-bootstrap install app \
  --manifest https://cocodeailabs.com/install-manifest.json \
  --json
```
...
```bash
cocodeai-bootstrap doctor --app --json
```
...
```bash
cocodeai-bootstrap cloud bootstrap-workspace \
  --base-url https://api.cocodeailabs.com \
  --workspace-name "<workspace-name>" \
  --skill-name "First CocodeAI Skill" \
  --claim-roles owner \
  --prepare-desktop \
...
1. Confirm in one line that CocodeAI is installed and their workspace is ready
   (use the workspace name).
2. Point them at ONE concrete first task they can run right now, e.g. "CocodeAI
...
4. End with this single question: "Want me to connect the CocodeAI MCP to this
...
1. A provisional workspace must be claimed before its owner can authenticate
   the CocodeAI MCP. If it has not been claimed yet, explain that dependency and
...
2. After the user has claimed the workspace and signed in, configure the MCP in
   ...
   ```text
   https://api.cocodeailabs.com/mcp/agent
   ```

3. For Codex, run:

   ```bash
   codex mcp add cocodeai --url https://api.cocodeailabs.com/mcp/agent
   codex mcp login cocodeai
   ```

   If an `cocodeai` entry already exists, do not add a duplicate. Authenticate
   the existing entry instead. To switch organizations or recover stale auth,
   run `codex mcp logout cocodeai` before `codex mcp login cocodeai`.
4. For another agent, use its current instructions from
   `https://cocodeailabs.com/docs/cloud/run-in-the-cloud/cloud-mcp`; do not guess
...
5. Tell the user to restart or reopen the current agent after setup so the new
   ...
   client can see CocodeAI's `search_capabilities` and `execute_capability`
   tools.
...
```bash
cocodeai-bootstrap cloud claim-link --role owner --json
```
...
- `cocodeai-bootstrap doctor --json` returns `ok: true`
- `cocodeai-bootstrap doctor --app --json` returns `ok: true`
- `cocodeai-bootstrap cloud bootstrap-workspace ... --json` returns:
  - `ok: true`
...
- `cocodeai-bootstrap doctor --desktop-bootstrap --json` returns `ok: true`
...
- If the `cocodeai-bootstrap` command is not found after install: ensure
  `$HOME/.local/bin` is on PATH, or call the binary by its full path
  (`$HOME/.local/bin/cocodeai-bootstrap`).
- If app install fails: run `cocodeai-bootstrap doctor --app --json` and report failed checks.
