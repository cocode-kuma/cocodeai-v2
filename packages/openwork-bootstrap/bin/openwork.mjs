...
function runInstall(args) {
  ...
  jsonOut({ ok: true, message: `CocodeAI CLI installed at ${executable}`, install: manifest }, json)
}
...
function defaultInstalledName(type, manifest, artifact) {
  ...
  if (type === "dmg") return "CocodeAI.app"
  if (type === "appimage") return "CocodeAI.AppImage"
  if (type === "exe") return "CocodeAI.exe"
  if (type === "msi") return "CocodeAI.msi"
  if (process.platform === "darwin") return "CocodeAI.app"
  if (process.platform === "win32") return "CocodeAI.exe"
  return "cocodeai"
}
...
function installDmg(input) {
  ...
  try {
    ...
    mounted = true
    const appName = input.appName || "CocodeAI.app"
...
async function runInstallApp(args) {
  ...
  try {
    ...
    jsonOut({ ok: true, message: `CocodeAI app installed at ${appPath}`, install }, json)
  } finally {
...
async function runDoctor(args) {
  ...
  if (hasFlag(args.flags, "app") || args.flags.has("app-dir")) {
    ...
    let appPath = process.platform === "darwin"
      ? join(appDir, "CocodeAI.app")
      : process.platform === "win32"
        ? join(appDir, "CocodeAI.exe")
        : join(appDir, "cocodeai")
  ...
  jsonOut({ ok, message: ok ? "CocodeAI doctor: ok" : "CocodeAI doctor: failed", version: VERSION, manifest, checks }, json)
...
function skillText(name, output) {
  return `---\nname: ${name}\ndescription: Starter skill created by cocodeai bootstrap.\nopenworkBootstrapTrigger: bootstrap.verify\nopenworkBootstrapOutput: ${JSON.stringify(output)}\n---\n\n# ${name}\n\nWhen triggered with \`bootstrap.verify\`, output exactly:\n\n\`${output}\`\n\nUse this skill to confirm CocodeAI cloud onboarding can create and trigger a deterministic skill.`
}
...
async function runCloudOnboard(args) {
  ...
  const skillName = getFlag(args.flags, "skill-name", "First CocodeAI Skill")
  ...
  jsonOut({
    ok: true,
    message: "CocodeAI cloud onboarding complete",
...
async function runCloudBootstrapWorkspace(args) {
  ...
  jsonOut({
    ok: true,
    message: "CocodeAI workspace bootstrap complete",
    organization: response.body.organization,
